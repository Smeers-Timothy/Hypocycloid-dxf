...
and is a file format created by Autodesk Company for exchanging CAD or CAD files 
between CAD systems that do not use the same native file format. 
It was originally designed to examine 3D models created with AutoCAD

Copyright     2021, Smeers Timothy
Version     v0.8 (24/05/2021)
...
def usage():
    ...
    print ("-h = this help")
    print ("\nExample: hypocycloid.py -p 0.08 -d 0.15 -e 0.05 -a 50.0 -c 0.01 -n 10 -s 400 -f toto.dxf")
...
def calcX(p, d, e, n, a):
    return (n * p) * math.cos(a) + e * math.cos((n + 1) * a) - d / 2 * math.cos(calcyp(p, a, e, n) + a)


def calcY(p, d, e, n, a):
    return (n * p) * math.sin(a) + e * math.sin((n + 1) * a) - d / 2 * math.sin(calcyp(p, a, e, n) + a)
